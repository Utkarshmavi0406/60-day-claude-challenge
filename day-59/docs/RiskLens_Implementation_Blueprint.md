...
## Day 6 — Implementation: SHAP Explainability (Complete)

**Completed today.** `src/explain.py` built a `RiskExplainer` class wrapping `final_model.pkl` with a `shap.TreeExplainer`, built once and reused (not rebuilt per prediction). Tested on 3 real dataset rows spanning the risk range: a 2.4% low-risk prediction (actual outcome: fully paid), a 91.6% high-risk prediction (actual outcome: defaulted), and a 15.0% borderline case — the model's predictions agreed with real recorded outcomes on both extremes.

**An honest limitation was found and documented, not hidden:** investigating the high-risk sample's factors surfaced that its DTI (1.22%, 3rd percentile — unusually low) had a *positive* SHAP value, meaning the model treats this specific applicant's low DTI as risk-*increasing*, counter to population-level intuition. This was verified directly (not assumed to be a bug) and documented in `reports/shap_sample_outputs.md` as a real, known property of tree-model SHAP values: row-specific, interaction-aware contributions can diverge from population averages, especially in sparse regions of the feature space.

`data/sample_applicants.json` was generated from 3 real dataset rows (low/high/medium risk), in the raw API-facing format — these become both the API's test fixtures and the frontend's quick-select buttons later.

<details>
<summary>Original Day 6 plan (for reference — superseded by the results above)</summary>
...
## Day 7 — Implementation: FastAPI Backend (Complete)

**Completed today, same session as SHAP** — the capstone template's Day 7 topic ("Product Refinement & UI") assumed an existing UI to polish, which didn't fit RiskLens's actual real build order (no frontend exists yet). Since there was nothing to polish, today accelerated to build both SHAP and the API instead, to reach an actually deployable state.

**Real, HTTP-level verification performed** (not just direct function calls): the server was started for real, and hit with actual `curl` requests.
- `GET /health` → `200 {"status":"ok","model_loaded":true}`
- `POST /predict` with a real high-risk profile → `200`, probability 0.9164, matching the direct-function test exactly
- Missing required fields → `422` with clear per-field detail
- Out-of-range value (`fico_score: 9999`) → `422` with a clear message
- Invalid enum value (`home_ownership: "SPACESHIP"`) → `422` listing the valid options
- A second valid profile (low-risk) → `200`, probability 0.0242, matching the direct-function test

**The encoding layer (`api/model_loader.py`) — the most bug-prone part of the whole system** — was verified by encoding all 3 sample applicants from their raw API format and confirming the resulting predictions exactly matched the predictions computed by feeding the model the original training-format rows directly. A silent column-order or one-hot mismatch here would have produced wrong predictions with no error — this was checked, not assumed.

**Handoff to the next content step (frontend):** `api/main.py`, `api/schemas.py`, `api/model_loader.py` are complete and tested. A frontend `fetch()` call to `POST /predict` needs no further backend changes — the contract is stable.

<details>
<summary>Original Day 7 plan (for reference — superseded by the results above)</summary>
...
## Day 8 — Implementation: Frontend (Complete)

**Completed today (capstone Day 9), alongside launch readiness work.** `frontend/index.html` built as a single self-contained file matching the established RiskLens brand identity from the Day 1 pitch deck (navy/ice/teal palette, Cambria display face) for visual consistency across the whole portfolio piece. Form covers all 15 applicant fields, with 3 real sample-applicant quick-select buttons (low/high/medium risk, from `data/sample_applicants.json`), a live results panel with a probability gauge, risk tier badge, and ranked SHAP factors.

**A real bug was found and fixed via actual browser testing, not just visual inspection:** `showState()` compared element IDs (`"errorState"`, `"loadingState"`) against short state names (`"error"`, `"loading"`) passed from call sites — those strings never matched, so both the loading spinner and the error state silently never appeared, even though the underlying logic (API calls, error catching) was completely correct. Found by deliberately triggering an aborted request in a real browser and tracing execution with injected console logging, which showed the catch block executing correctly but the display state never actually changing. Fixed by using the exact element ID strings as the state names everywhere. Reconfirmed working: error state now displays correctly on a failed request, and the loading spinner was confirmed visible during an artificially-delayed real request.

**Full integration verified**, not just the frontend in isolation: `api/main.py` now mounts `frontend/` as static files, served from the same origin as `/predict` and `/health` — tested by starting the real server and confirming the frontend loads at `/`, `data/sample_applicants.json` is reachable, and a full prediction request-response cycle works identically to the API-only tests from Day 7/8 (91.64% probability, exact match).

<details>
<summary>Original Day 8 plan (for reference — superseded by the results above)</summary>
...
> **Note added after a Day 8 capstone session:** A full release-readiness review (Senior QA/Security/Performance lens) was already performed on everything that exists so far — the data pipeline, both models, SHAP explainability, and the FastAPI backend. See `docs/testing_checklist.md` for the complete results: 2 real bugs found and fixed (missing validation upper bounds, a missing cross-field logical constraint), 1 real code-quality issue found and fixed (feature-list duplication across 3 files, consolidated into `src/feature_config.py`), plus a documented performance/security review. **This Day 9 section still applies in full once the frontend exists** — today's pass covered everything that existed *before* the frontend, not the frontend itself, which hasn't been built yet.
