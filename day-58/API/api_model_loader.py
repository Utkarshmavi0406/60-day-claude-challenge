...
from feature_config import MODEL_FEATURE_ORDER, TERM_COLUMN  # noqa: E402

# MODEL_FEATURE_ORDER now imported from feature_config.py — single source
# of truth (Day 8 QA fix: this list was previously hand-duplicated here,
# in clean_data.py, and in train_baseline.py — a real maintainability
# risk, since a future feature change forgotten in even one of the three
# would silently reintroduce a column-order mismatch bug).
...
def encode_applicant(applicant) -> pd.DataFrame:
    ...
    row[TERM_COLUMN] = 1 if applicant.term_60_months else 0
